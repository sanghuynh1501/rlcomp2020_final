...
class MapInfo:
    ...
    def gold_amount_y(self, y, player_object, player_count):
        for cell in self.golds:
            if cell["posy"] == y:
                if str(cell["posx"]) + "_" + str(cell["posy"]) not in player_object:
                    return True
                else:
                    if player_object[str(cell["posx"]) + "_" + str(cell["posy"])] == player_count:
                        return True
        return False

    def gold_amount_x(self, x, player_object, player_count):
        for cell in self.golds:
            if cell["posx"] == x:
                if str(cell["posx"]) + "_" + str(cell["posy"]) not in player_object:
                    return True
    ...
    def get_gold_pos(self, pos):
        results = []
        for cell in self.golds:
            if pos[0] == cell["posx"] or pos[1] == cell["posy"]:
                results.append([cell["posx"], cell["posy"]])
        return results
    ...
    def get_obstacle_swamp_object(self):
        swamp_object = {}
        obstacle_object = {}
        for cell in self.obstacles:
            if cell["type"] == SwampID:
                swamp_object[str(cell["posx"]) + "_" + str(cell["posy"])] = abs(cell["value"])
            else:
                obstacle_object[str(cell["posx"]) + "_" + str(cell["posy"])] = cell["type"]
        return swamp_object, obstacle_object

    def get_gold_amount_object(self):
        gold_object = {}
        for cell in self.golds:
            gold_object[str(cell["posx"]) + "_" + str(cell["posy"])] = cell["amount"]
        return gold_object

    def get_obstacle_swamp(self, x, y):  # Get the kind of the obstacle at cell(x,y)
        for cell in self.obstacles:
            if x == cell["posx"] and y == cell["posy"]:
                if cell["type"] == SwampID:
